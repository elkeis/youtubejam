...
export class ProcessingService {
  ...
  async createProcessing(inputFileName: string): Promise<ProcessingDto> {
    try {
      const newProcessing = new this.processingModel(
      ...
      throw new InternalServerErrorException(
        e,
        `Error during create processing for ${inputFileName}`,
      );
    }
  }
  ...
  async updateWithProgress(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        e,
        `Error during updating progress for ${processingId} processing`,
      );
    }
  }
  ...
  async updateWithError(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        e,
        `Error during setting an error for ${processingId} processing`,
      );
    }
  }
    ...
    try {
      ...
      if (!processingDocument) {
        throw new NotFoundException(
          `Can't find any processing with id ${processingId}`,
        );
      }
      ...
      throw e instanceof HttpException
        ...
        : new InternalServerErrorException(
            e,
            `Error during fetching processing with ${processingId} processingId`,
          );
    }
  }
}
